/*
	Modelo: Problema de Fluxo Máximo (PFM)
...
int s, t; // Nó de origem e de destino
...
void cplex(){
	...
	for( i = 0; i < N; i++ ){
		...
		for( j = 0; j < N; j++ ){
            ...
            if(arestas[i][j] != -1) {
                x[i].add(IloIntVar(env, 0, arestas[i][j]));
                numberVar++;
            }
	...
	for(i=0; i<N; i++){ // For que representa o (Para Todo).
		...
		for( j = 0; j < N; j++ ){
			// Verifica se a aresta (i, j) existe e se i não é a origem:
			if(arestas[i][j] != -1 && i != s) {
                sum += x[i][j];
            }
		}
		...
		for( k = 0; k < N; k++ ){
			// Verifica se a aresta (k, i) existe e se i não é o destino:
			if(arestas[k][i] != -1 && i != t) {
                sum2 += x[k][i];
            }
		}
...
int main(){
	...
	// Inicializa a capacidade cada aresta com -1:
	for(i=0; i<N; i++) {
		for(j=0; j<N; j++) {
			arestas[i][j] = -1;
		}
    ...
    printf("Nó de origem e de destino: %d %d\n", s, t);
	printf("Arestas - id_i - id_j: capacidade\n");
    for(i=0; i<N; i++) {
        for(j=0; j<N; j++) {
			// Verifica se a aresta (i, j) existe:
            if(arestas[i][j] != -1) {
